...
def add_embedding_openai(doc: Document, nb_openai_used_tokens: int):
    ...
    for idx_sub_doc in tqdm(range(0, len(doc_ai.sub_documents), 20)):
        results = get_embedding([doc_.text for doc_ in doc_ai.sub_documents[idx_sub_doc:idx_sub_doc+20]], model=OPENAI_MODEL_EMBEDDING)
...
def save_embeddings(embeddings):
    ...
    with open(
        f"../data/processed/province-sud/BaieDesCitrons_barriere_01062023_{str_date}.pickle", "wb"
    ) as handle:
